...
function makeLane(cars, roadY, offsetY, dir, speed, count, turning = false, laneIdx = 0) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const startX = dir === 1 ? (i * spacing) - 60 : W - (i * spacing) + 60;
    cars.push({
...
function buildRoad(config) {
  ...
  return {
    ...
    numLanes: numLanes,
    crosswalkX: config.crosswalkX,
    hasLight: config.hasLight,
    durations: config.durations || { green: 4000, yellow: 1000, red: 4200 },
    phase: 'green',
    phaseTimer: 0,
    cars,
    state: 'below',
  ...
  switch (n) {
    case 1:
      return {
        ...
        mission: 'รอไฟแดง แล้วข้ามทางม้าลายอย่างปลอดภัย',
        timeLimit: 90,
        roads: [buildRoad({
          y: 320, height: 90, crosswalkX: [380, 580], hasLight: true,
          durations: { green: 4200, yellow: 1100, red: 4600 },
          lanes: [
    ...
    case 2:
      return {
        name: 'ด่าน 2: ทางม้าลายไร้สัญญาณไฟ',
        mission: 'ยืนตรงทางม้าลาย แล้วกด [Spacebar] หรือปุ่ม [✋] เพื่อยกมือขอทาง!',
        timeLimit: 80,
        roads: [buildRoad({
          y: 320, height: 90, crosswalkX: [380, 580], hasLight: false,
          lanes: [
    ...
    case 3:
      return {
        name: 'ด่าน 3: ข้ามถนนแบบปลอดภัย',
        mission: 'ข้าม 2 ถนน  และยกมือขอทางที่ทางม้าลายเมื่อไม่มีสัญญาญไฟ',
        timeLimit: 85,
        roads: [
          buildRoad({
          ...
          buildRoad({
            y: 420, height: 90, crosswalkX: [380, 580], hasLight: false,
            lanes: [
    ...
    case 4:
      return {
        ...
        mission: 'ข้ามถนนหนาแน่น สังเกตจังหวะรถให้ดี',
        timeLimit: 85,
        roads: [
          buildRoad({
          ...
          buildRoad({
            y: 430, height: 90, crosswalkX: [370, 590], hasLight: false,
            lanes: [
    ...
    case 5:
      return {
        ...
        mission: 'ข้ามให้ครบ 3 ถนน โดยใช้ทางม้าลาย',
        timeLimit: 100,
        roads: [
          buildRoad({
          ...
          buildRoad({
            y: 320, height: 80, crosswalkX: [380, 580], hasLight: false,
            lanes: [
...
function hintForLevel(n) {
  ...
  switch (n) {
    case 1: return base + 'รอไฟแดงคนข้าม';
    case 2: return base + 'ยืนที่ทางม้าลาย แล้วกด Spacebar ค้างไว้เพื่อยกมือขอทาง!';
    case 3: return base + 'ระวังรถและยกมือเพื่อขอทางไม่มีสัญญาณไฟ';
    case 4: return base + 'รถเยอะมาก! ชูมือขอทางเพื่อให้รถชะลอจอด';
    case 5: return base + 'ด่านสุดท้าย! มีสติ ใช้ทางม้าลาย และชูมือทุกครั้ง';
  }
  return base;
}
  ...
  road.cars.forEach(car => {
    ...
    if (road.hasLight) {
      ...
      if (distToCrosswalk > 0 && distToCrosswalk < 220) {
        targetSpeed = distToCrosswalk <= 8 ? 0 : car.baseSpeed * (distToCrosswalk / 220);
      }
    }
    ...
    road.cars.forEach(other => {
      if (other === car || other.dir !== car.dir) return;
      if (Math.abs(other.y - car.y) > 10) return;
    ...
    if (aheadCar) {
      const safeGap = (car.w / 2 + aheadCar.w / 2) + 16;
      ...
      if (gap <= 0) {
        ...
        const followSpeed = Math.min(aheadCar.speed, car.baseSpeed * (gap / 80));
        targetSpeed = Math.min(targetSpeed, followSpeed);
      }
    }

    car.speed += (targetSpeed - car.speed) * Math.min(1, dt * 7);
    car.x += car.dir * car.speed * dt;

    if (car.dir === 1 && car.x - car.w / 2 > W + 60) {
      const isBlocked = road.cars.some(o => o !== car && Math.abs(o.y - car.y) < 10 && o.x < 100);
      if (!isBlocked) {
        car.x = -60;
      ...
      const isBlocked = road.cars.some(o => o !== car && Math.abs(o.y - car.y) < 10 && o.x > W - 100);
      if (!isBlocked) {
        car.x = W + 60;
        car.vData = getRandomVehicleType();
        car.w = car.vData.w;
...
function finalizeCrossing(road) {
  ...
  if (cs.usedCrosswalk && cs.raisedHand && !road.hasLight) {
    gain = 150; msg = 'สุดยอด! ยกมือขอข้ามทางม้าลายอย่างปลอดภัย 🇹🇭'; cls = 'good'; SFX.good();
    spawnParticles(game.player.x, game.player.y, 20, '#ffd700', 1.2);
  } else if (cs.usedCrosswalk && cs.safeStart) {
    gain = 100; msg = 'คุณตัดสินใจได้ถูกต้อง! ข้ามทางม้าลายอย่างปลอดภัย'; cls = 'good'; SFX.good();
    spawnParticles(game.player.x, game.player.y, 15, '#4caf50', 1);
  } else if (cs.usedCrosswalk && !cs.safeStart) {
    gain = 30; msg = 'ข้ามได้แต่เสี่ยง! ควรกด Spacebar ชูมือขอทางหรือรอจังหวะรถว่าง'; cls = 'bad'; SFX.bad();
  } else {
    gain = 10; msg = 'ควรใช้ทางม้าลายทุกครั้งเพื่อความปลอดภัย'; cls = 'bad'; SFX.bad();
  }
  game.score += gain;
...
function endLevel() {
  ...
  document.getElementById('nextLevelBtn').textContent = game.levelIndex >= TOTAL_LEVELS ? 'ดูผลรวม' : 'ต่อไป';
...
function draw() {
  ...
  drawBackground();        // ทางเท้ากระเบื้องเมืองไทย
  drawWalkways();          // ลานกระเบื้องเชื่อมจากประตูโรงเรียน
  drawGoalZone();          // โรงเรียน
  drawHomeZone();
...
/* --- ลานกระเบื้องเชื่อมต่อจากประตูโรงเรียน --- */
...
function drawRoad(road) {
  ...
  // --- ทางม้าลายแบบประเทศไทย (แถบสีขาวแนวตั้งตามทิศทางการข้ามถนน เหมือนภาพที่ 3) ---
  ...
  // พื้นสีแดงของทางม้าลาย
  ctx.fillStyle = 'rgba(211, 47, 47, 0.85)';
  ctx.fillRect(cx0, crossY, cwWidth, crossH);

  // แถบม้าลายสีขาวแนวตั้ง (ซอยเส้นให้เยอะขึ้น)
  const stripeW = 10; // ความกว้างของแต่ละแถบแนวตั้ง
  const gapX = 8;     // ระยะห่างระหว่างแถบ
  const totalUnitX = stripeW + gapX;
  const countX = Math.floor(cwWidth / totalUnitX);
  const totalUsedW = (countX * stripeW) + ((countX - 1) * gapX);
  const startX = cx0 + (cwWidth - totalUsedW) / 2;

  for (let i = 0; i < countX; i++) {
    const sx = startX + i * totalUnitX;
    const stripeGrad = ctx.createLinearGradient(sx, crossY, sx, crossY + crossH);
    stripeGrad.addColorStop(0, '#ffffff');
    stripeGrad.addColorStop(1, '#dcdcdc');
    ctx.fillStyle = stripeGrad;
    ctx.fillRect(sx, crossY, stripeW, crossH);
    ctx.fillStyle = 'rgba(0,0,0,0.12)';
    ctx.fillRect(sx + stripeW - 2, crossY, 2, crossH);
  }

  if (road.hasLight) drawTrafficLight(cx1 + 26, top - 8, road);
}
...
function drawGoalZone() {
  ...
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('🏫 โรงเรียนวิทยาความปลอดภัย', cx, signY + signH / 2);
  ...
  // --- ปรับตำแหน่งธงชาติไทยลงมาให้อยู่ในกรอบมองเห็นเต็มผืน ---
  const flagX = cx - 150;
  const flagY = bY + 18; 
...
function updateHUD() {
  ...
  document.getElementById('timeValue').textContent = `${pad(Math.floor(t / 60))}:${pad(t % 60)}`;
...
function setHint(text) { document.getElementById('hintPanel').textContent = text; }
...
function showScreen(id) {
  ...
  const isPlayingOrPaused = (state === STATE.PLAYING || state === STATE.PAUSED);
  document.getElementById('hud').classList.toggle('hidden', !isPlayingOrPaused);
  
  const isTouchDevice = ('ontouchstart' in window || navigator.maxTouchPoints > 0);
  const touchControls = document.getElementById('touchControls');
  if (touchControls) {
    touchControls.classList.toggle('hidden', !(isTouchDevice && isPlayingOrPaused));
  }
}

function togglePause() {
  if (state === STATE.PLAYING) { state = STATE.PAUSED; SFX.click(); showScreen('pauseScreen'); }
  else if (state === STATE.PAUSED) { state = STATE.PLAYING; SFX.click(); showScreen(null); }
}
...
document.getElementById('startBtn').addEventListener('click', () => {
  ensureAudio(); SFX.click(); startNewGame(); state = STATE.PLAYING; showScreen(null);
});
...
document.getElementById('exitBtn').addEventListener('click', () => {
  ...
  document.getElementById('stage').innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#fff;font-size:1.2rem;background:#0d1420;">ขอบคุณที่เล่นเกม 🙏 ข้ามถนนอย่างปลอดภัยนะครับ</div>';
});
...
document.getElementById('restartFromPauseBtn').addEventListener('click', () => {
  ensureAudio(); SFX.click(); loadLevel(game.levelIndex); state = STATE.PLAYING; showScreen(null);
});
document.getElementById('menuFromPauseBtn').addEventListener('click', () => { ensureAudio(); SFX.click(); goMenu(); });
...
document.getElementById('nextLevelBtn').addEventListener('click', () => {
  ensureAudio(); SFX.click();
  if (game.levelIndex >= TOTAL_LEVELS) { endGame(true); return; }
...
document.getElementById('mainMenuBtn').addEventListener('click', () => { ensureAudio(); SFX.click(); goMenu(); });
document.getElementById('playAgainBtn').addEventListener('click', () => {
  ensureAudio(); SFX.click(); startNewGame(); state = STATE.PLAYING; showScreen(null);
});
document.getElementById('winMenuBtn').addEventListener('click', () => { ensureAudio(); SFX.click(); goMenu(); });
