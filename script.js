...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
let audioCtx = null;
let soundOn = true;   // ปุ่มเสียงเอฟเฟกต์ (SFX)
let musicOn = true;   // ปุ่มเพลงพื้นหลัง (แยกอิสระจากเสียงเอฟเฟกต์)
...
function ensureAudio() {
  ...
  // เริ่มเปิดเพลงพื้นหลังทันทีที่มี user gesture ครั้งแรก (เบราว์เซอร์กันไม่ให้เล่นเสียงเองโดยไม่มีการโต้ตอบ)
  // เล่นเฉพาะเมื่อผู้เล่นเปิดเพลงไว้ (musicOn) เท่านั้น
  if (musicOn && typeof Music !== 'undefined' && !Music.isPlaying) {
    Music.start();
  }
}

/* พยายามเล่นเพลงทันทีตั้งแต่โหลดหน้าเสร็จ (ไม่ต้องรอกดปุ่มในเกม)
   เบราว์เซอร์บางตัวอนุญาตถ้าไม่มีนโยบายเข้มงวด แต่ถ้าเบราว์เซอร์บล็อก (ส่วนใหญ่จะบล็อก)
   จะไปเล่นทันทีที่ผู้เล่นแตะ/คลิก/กดปุ่มอะไรก็ได้บนหน้าจอครั้งแรก (ไม่ต้องเจาะจงว่าต้องกดปุ่มเกม) */
document.addEventListener('DOMContentLoaded', () => {
  if (musicOn) Music.start();
});
['pointerdown', 'touchstart', 'keydown'].forEach(evt => {
  window.addEventListener(evt, () => { ensureAudio(); }, { capture: true, passive: true });
});
...
/* ---------------------------------------------------------------------
   BACKGROUND MUSIC (เล่นจากไฟล์เพลง Morning_Traffic_High_Score.mp3
   วนซ้ำตลอดเกม)
   --------------------------------------------------------------------- */
const Music = (() => {
  let playing = false;
  ...
  function getAudio() {
    if (!audioEl) {
      audioEl = new Audio('Morning_Traffic_High_Score.mp3');
      audioEl.loop = true;
      audioEl.volume = musicOn ? VOLUME : 0;
  ...
  switch (n) {
    ...
    case 2:
      return {
        ...
        mission: 'ถนนนี้ไม่มีไฟจราจร กด [Spacebar] หรือ [ขอมือ] เพื่อยกมือขอทางที่ทางม้าลาย',
        timeLimit: 80,
        roads: [buildRoad({
    ...
    case 3:
      return {
        ...
        mission: 'ถนนแรกมีสัญญาณไฟ ส่วนถนนที่ 2 ไม่มีไฟ ให้ใช้ทางม้าลายและยกมือขอทาง',
        timeLimit: 85,
        roads: [
          buildRoad({
    ...
    case 4:
      return {
        ...
        mission: 'ถนนทั้ง 2 สายไม่มีไฟจราจร ใช้ทางม้าลาย ยกมือขอทาง และรอให้รถหยุด',
        timeLimit: 85,
        roads: [
          buildRoad({
            y: 210, height: 90, crosswalkX: [370, 590], hasLight: false,
    ...
    case 5:
      return {
        ...
        mission: 'ข้ามให้ครบ 3 ถนน ถนนกลางไม่มีไฟจราจร ให้ใช้ทางม้าลายและยกมือขอทาง',
        timeLimit: 100,
        roads: [
          buildRoad({
...
function updateRoad(road, dt) {
  ...
  road.cars.forEach(car => {
    ...
    // ถนนที่มีไฟจราจร: การชูมือขอทางไม่มีผลต่อการหยุดรถ (รถจะหยุดตามสัญญาณไฟเท่านั้น)
    // ถนนที่ไม่มีไฟจราจร: การชูมือขอทางยังคงช่วยให้รถชะลอ/หยุดให้ข้ามได้ตามเดิม
    if (!road.hasLight && isPlayerRaisingHand) {
...
function finalizeCrossing(road) {
  ...
  if (road.hasLight) {
    if (cs.usedCrosswalk && cs.raisedHand && road.phase === 'red') {
      gain = 150; msg = 'ถูกต้อง! ข้ามทางม้าลายตามสัญญาณไฟปลอดภัย'; cls = 'good'; SFX.good();
      ...
      gain = 30; msg = 'ข้ามได้แต่เสี่ยง! ควรรอไฟแดงก่อนค่อยข้ามถนน'; cls = 'bad'; SFX.bad();
    } else {
      gain = 10; msg = 'ควรใช้ทางม้าลายและสังเกตสัญญาณไฟทุกครั้ง'; cls = 'bad'; SFX.bad();
    }
  } else {
    // ถนนไม่มีไฟจราจร: ต้องใช้ทางม้าลายและยกมือขอทางเพื่อได้คะแนนเต็ม
    if (cs.usedCrosswalk && cs.raisedHand) {
      gain = 130; msg = 'เยี่ยม! ใช้ทางม้าลายและยกมือขอทางอย่างปลอดภัย'; cls = 'good'; SFX.good();
      spawnParticles(game.player.x, game.player.y, 20, '#ffd700', 1.2);
    } else if (cs.usedCrosswalk && !cs.raisedHand) {
      gain = 40; msg = 'ข้ามทางม้าลายได้ แต่ควรยกมือขอทางทุกครั้งที่ไม่มีไฟจราจร'; cls = 'bad'; SFX.bad();
    } else if (!cs.usedCrosswalk && cs.raisedHand) {
      gain = 20; msg = 'ยกมือขอทางดีแล้ว แต่ควรข้ามบนทางม้าลายด้วย'; cls = 'bad'; SFX.bad();
    } else {
      gain = 10; msg = 'ควรใช้ทางม้าลายและยกมือขอทางทุกครั้งที่ไม่มีไฟจราจร'; cls = 'bad'; SFX.bad();
    }
  }
  game.score += gain;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ----- ปุ่มเปิด/ปิดเสียงเอฟเฟกต์ (SFX) ใช้ร่วมกันทั้งหน้าเมนูและในเกม -----
  function toggleSound() {
    ensureAudio();
    soundOn = !soundOn;
    ['soundBtn', 'menuSoundBtn'].forEach(id => {
      const btn = document.getElementById(id);
      if (btn) {
        btn.textContent = soundOn ? '🔊' : '🔇';
        btn.title = soundOn ? 'ปิดเสียงเอฟเฟกต์' : 'เปิดเสียงเอฟเฟกต์';
      }
    });
    if (soundOn) SFX.click();
  }
  bindClick('soundBtn', toggleSound);
  bindClick('menuSoundBtn', toggleSound);

  // ----- ปุ่มเปิด/ปิดเพลงพื้นหลัง (แยกอิสระจากปุ่มเสียงเอฟเฟกต์ด้านบน) -----
  function toggleMusic() {
    ensureAudio();
    musicOn = !musicOn;
    Music.setMuted(!musicOn);
    if (musicOn && audioCtx && !Music.isPlaying) Music.start();
    ['musicBtn', 'menuMusicBtn'].forEach(id => {
      const btn = document.getElementById(id);
      if (btn) {
        btn.textContent = '🎵';
        btn.classList.toggle('is-off', !musicOn);
        btn.title = musicOn ? 'ปิดเพลง' : 'เปิดเพลง';
      }
    });
    if (soundOn) SFX.click();
  }
  bindClick('musicBtn', toggleMusic);
  bindClick('menuMusicBtn', toggleMusic);
