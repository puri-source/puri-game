...
function drawRoad(road) {
  ...
  // --- ทางม้าลาย (แถบสีแดง-ขาว เยอะขึ้น อยู่ในกรอบ เริ่มด้วยสีแดง และจบด้วยสีแดง) ---
  const [cx0, cx1] = road.crosswalkX;
  const cwWidth = cx1 - cx0;
  const padY = 2; // เว้นขอบนิดหน่อยเพื่อให้อยู่ในกรอบถนนสวยงาม
  const crossH = road.height - (padY * 2);
  const crossY = top + padY;

  // กำหนดจำนวนเส้นทั้งหมดเป็น 13 เส้น (ต้องเป็นจำนวนคี่เพื่อให้เริ่มด้วยแดง และจบด้วยแดง)
  const stripeCount = 13;
  const stripeH = crossH / stripeCount; // คำนวณความสูงให้พอดีเป๊ะในกรอบ
  ...
  for (let i = 0; i < stripeCount; i++) {
    ...
    // เงาบางๆ ใต้แต่ละแถบเพื่อเพิ่มมิติ
